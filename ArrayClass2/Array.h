/*
    Array class for searching and sorting
...
// Random values in the array between 1 and this limit
#define MAX 5000
// Random spacing between ordered numbers
#define STEP 100
...
class Array {
    private:
        ...
        void mergeSortRecursion(int * temp, int start, int end);
        void mergeSortMerge(int * temp, int start, int middle, int end);
...
template <class T>
void Array<T>::fillOrdered()
{
    // Start at a random number
    data[0] = rand() % STEP + 1;
    for (int i=1; i<size; i++)
    {
        // Add a random amount to the previous item in the array
...
template <class T>
int Array<T>::linearSearch(int item)
{
    for (int i=0; i<size; i++)
    {
        if (data[i] == item)
...
// Return -1 if not found, otherwise return the index where item was found
template <class T>
int Array<T>::binarySearchIterative(int item)
{
